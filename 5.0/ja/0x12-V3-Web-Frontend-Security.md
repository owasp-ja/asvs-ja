...
| # | 説明 | レベル | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **3.6.1** | JavaScript ライブラリ、CSS、Web フォントなどのクライアントサイド資産がコンテンツ配信ネットワーク (Content Delivery Network, CDN) や外部プロバイダで外部的にホストされている場合、サブリソース完全性 (Subresource Integrity, SRI) を使用して資産の完全性を確認している。 | 3 | v5.0.be-50.7.1 |
