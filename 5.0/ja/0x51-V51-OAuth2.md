...
| # | 説明 | L1 | L2 | L3 |
| :---: | :--- | :---: | :---: | :---: |
| **51.1.1** | [追加] 認可サーバは PKCE が提供するメカニズム「nonce」パラメータまたは「state」パラメータのいずれかを使用して CSRF 保護をサポートしている。 | ✓ | ✓ | ✓ |
| **51.1.2** | [追加] 認可レスポンスへの認可コードのリプレイは PKCE フローを使用するか、OpenID Connect の「nonce」パラメータと ID Token のそれぞれの Claim を使用することで防止している。PKCE チャレンジや OpenID Connect の「nonce」はトランザクション固有であり、トランザクションが開始されたクライアントとユーザエージェントに安全にバインドされなければならない。 | ✓ | ✓ | ✓ |
| **51.1.3** | [追加] 認可サーバは、認可リクエストに「code_challenge」パラメータが存在する場合にのみ「code_verifier」パラメータを含むトークンリクエストを受け付けるようにすることで、PKCE ダウングレード攻撃を緩和している。
| **51.1.4** | [追加] リフレッシュトークンは送信者制約があるか、リフレッシュトークンローテーションを使用して、トークンリプレイ攻撃を防止している。リフレッシュトークンローテーションはリフレッシュトークンが侵害された場合の使用を防止する。送信者制約があるリフレッシュトークンはリフレッシュトークンを特定のクライアントに暗号的にバインドする。 | ✓ | ✓ | ✓ |
| **51.1.5** | [追加] クライアントが認可リクエストで有効な PKCE の「code_challenge」パラメータを送信する場合、認可サーバはトークンエンドポイントで「code_verifier」の正しい使用を強制している。 | ✓ | ✓ | ✓ |
| **51.1.6** | [追加] リソースオーナーのパスワードクレデンシャル付与は認可サーバによって使用または構成されていない。この付与タイプはリソースオーナーのクレデンシャルをクライアントに安全に公開しないため、アプリケーションの攻撃対象領域が増大する。 | ✓ | ✓ | ✓ |
| **51.1.7** | [追加] 認可リクエスト内のリダイレクト URI は HTTPS スキームを使用する絶対的なものであり、事前登録された URI のクライアント固有の許可リストに基づいて、正確な文字列比較を使用して認可サーバによって検証されている。 | ✓ | ✓ | ✓ | 601 |

## V51.2 OAuth クライアント

| # | 説明 | L1 | L2 | L3 |
| :---: | :--- | :---: | :---: | :---: |
| **51.2.1** | [追加] OAuth クライアントが複数の認可サーバとやり取りできる場合、クライアントは発行者の「iss」パラメータ値が認可レスポンスから期待されるものであることを検証して、ミックスアップ攻撃を防御している。「iss」パラメータがない場合、クライアントは代わりに別のリダイレクト URI を使用して、認可エンドポイントとトークンエンドポイントを識別できる。 | ✓ | ✓ | ✓ |
| **51.2.2** | [追加] クライアントは PKCE フローを使用するか、あるいは OpenID Connect の「nonce」パラメータと ID Token のそれぞれの Claim を使用している。 | ✓ | ✓ | ✓ |
| **51.2.3** | [追加] クライアントは「scope」パラメータと「resource」パラメータを利用して、それぞれアクセスしたいリソースサーバを決定している。 | ✓ | ✓ | ✓ |
| **51.2.4** | [追加] クライアントは「scope」パラメータと「authorization_details」パラメータを利用して、アクセストークンが制限される関連リソースとアクションを決定している。 | ✓ | ✓ | ✓ |

## V51.3 リソースサーバ

| # | 説明 | L1 | L2 | L3 |
| :---: | :--- | :---: | :---: | :---: |
| **51.3.1** | [追加] リソースサーバは送信者制約があるアクセストークンメカニズムを実装して、トークンリプレイ攻撃を緩和している。これは、OAuth 2.0 または OAuth Demonstration of Proof of Possession (DPoP) に相互 TLS を義務付け、クライアント登録時に提供されるクライアントシークレットを使用することで実現されている。 | ✓ | ✓ | ✓ |
| **51.3.2** | [追加] アクセストークンは特定のリソースサーバ、できれば単一のリソースサーバに制限されている (利用者制限)。すべてのリソースサーバは、リクエストごとに、そのリクエストとともに送信されるアクセストークンがその特定のリソースサーバに使用されることを意図したものであるかどうかを検証する義務がある。そうでない場合、リソースサーバはそれぞれのリクエストへの対処を拒否しなければならない。 | ✓ | ✓ | ✓ |
