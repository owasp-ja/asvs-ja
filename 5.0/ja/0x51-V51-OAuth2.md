...
| **51.2.3** | [追加] コードフローが使用される場合、認可サーバは PKCE の使用を要求して、認可コード傍受攻撃を軽減している。認可リクエストでは、認可サーバは有効な code_challenge 値を要求し、code_challenge_method 値 S256 (プレーンは許可されない) のみを受け入れなければならない。トークンリクエストでは、code_challenge から計算される値を持つ code_verifier を要求しなければならない。 | ✓ | ✓ | ✓ |
...
| **51.2.5** | [追加] リソースオーナーのパスワードクレデンシャル付与は認可サーバによって使用または構成されていない。この付与タイプはリソースオーナーのクレデンシャルをクライアントに安全に公開しないため、アプリケーションの攻撃対象領域が増大する。 | ✓ | ✓ | ✓ |
