...
| **51.1.7** | [追加] 認可サーバは、正確な文字列比較を使用して、事前登録された URI のクライアント固有の許可リストに基づいて、リダイレクト URI を検証している。 | ✓ | ✓ | ✓ |
...
| **51.2.5** | [追加] コードフローが使用される場合、OAuth クライアントは PKCE 機能を使用するか、認可リクエストで送信された state パラメータをチェックすることで、トークンリクエストをトリガーする CSRF 攻撃から保護している。 | ✓ | ✓ | ✓ |
