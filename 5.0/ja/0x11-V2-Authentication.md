...
| :---: | :--- | :---: | :---: | :---: | :---: | :---: |
| **2.1.1** | [修正] ユーザが設定するパスワードは、最低 8 文字となっている。 | ✓ | ✓ | ✓ | 521 | 5.1.1.2 |
...
| **2.1.8** | [削除, 不十分な影響] | | | | | |
| **2.1.9** | 使用可能な文字の種類を制限するパスワード規則がない。大文字、小文字、数字、特殊文字を要求する必要はない。 | ✓ | ✓ | ✓ | 521 | 5.1.1.2 |
...
| **2.1.14** | [追加, 2.1.7 から分割, レベル L1 > L3] アカウント登録またはパスワード変更時に送信されるパスワードは一連の侵害されたユーザ名とパスワードのペアと照合されている。 | | | ✓ | | 5.1.1.2 |
...
| **2.4.2** | [削除] | | | | | |
| **2.4.3** | [修正] PBKDF2 が使用されている場合、反復回数は PBKDF2-HMAC-SHA1 では 1,300,000 回以上、PBKDF2-HMAC-SHA256 では 600,000 回以上、PBKDF2-HMAC-SHA512 では 210,000 回以上とする。 | | ✓ | ✓ | 916 | 5.1.1.2 |
...
| **2.5.5** | [削除, 2.2.3 と重複] | | | | | |
| **2.5.6** | パスワードを忘れた場合や他のリカバリパスは、時間ベースの OTP (time-based OTP, TOTP) またはその他のソフトトークン、モバイルパス、または別のオフラインリカバリ機構などのセキュアなリカバリ機構を使用する。 | ✓ | ✓ | ✓ | 640 | 5.1.1.2 |
...
| **2.10.5** | [9.2.3 から移動] センシティブな情報や機能を持つ外部システムとの暗号化接続がすべて認証されている。 | | ✓ | ✓ | 287 |
