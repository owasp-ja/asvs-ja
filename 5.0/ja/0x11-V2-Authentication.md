...
| # | 説明 | レベル | CWE |
| :---: | :--- | :---: | :---: |
| **2.6.1** | ルックアップシークレットは一度だけしか使用されない。 | 2 | 308 |
...
| **2.6.3** | [修正] ルックアップシークレットは予測可能な値を避けるために暗号的に安全な疑似乱数ジェネレータ (Cryptographically Secure Pseudorandom Number Generator, CSPRNG) を使用して生成されている。 | 2 | 310 |
...
| **2.7.2** | [修正] 経路外認証リクエスト、コード、またはトークンは 10 分以内に期限切れになる。 | 1 | 287 |
| **2.7.3** | [文法, 2.2.6 をカバー] 経路外認証リクエストやコード、またはトークンが 1 回しか使用できず、元の認証リクエストに対してのみ使用可能となっている。 | 1 | 287 |
| **2.7.4** | [文法] 使用されているセカンダリ通信チャネルは安全であり、プライマリチャネルから独立している。 | 1 | 523 |
| **2.7.5** | [削除, 不十分な影響] | | |
| **2.7.6** | [修正] 経路外認証で使用されるコードは暗号的に安全な乱数生成器 (CSPRNG) を使用して生成され、少なくとも 20 ビットのエントロピーを含む (通常は 4 つのランダムな英数字または 6 つのランダムな数字で十分である)。 | 2 | 310 |
...
| **2.8.3** | [文法] 承認済みの暗号化アルゴリズムが時間ベースのワンタイムパスワードの生成、シード、検証に使用されている。 | 2 | 326 |
| **2.8.4** | [文法, 2.2.6 をカバー] 時間ベースのワンタイムパスワードが有効期間内に 1 回しか使用できない。 | 2 | 287 |
| **2.8.5** | [削除, 不十分な影響] | | |
