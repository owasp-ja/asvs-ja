...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **2.9.1** | [修正, レベル L2 > L3] 認証検証者は、検証に使用される暗号鍵を、変更 (対象鍵の場合は開示) から保護されるように保存している。これには、トラステッドプラットフォームモジュール (Trusted Platform Module, TPM)、ハードウェアセキュリティモジュール (Hardware Security Module, HSM)、またはこの安全なストレージを提供できる OS サービスを使用できる。 | | | ✓ | 320 |
