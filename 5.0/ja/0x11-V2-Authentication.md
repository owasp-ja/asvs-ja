...
| :---: | :--- | :---: | :---: | :---: | :---: |
| **2.5.1** | [削除, 不正確] | | | | |
| **2.5.2** | パスワードのヒントや知識ベースの認証（いわゆる「秘密の質問」）が存在しない。 | ✓ | ✓ | ✓ | 640 |
| **2.5.3** | [削除, 2.4.1 と重複] | | | | |
| **2.5.4** | [14.1.10 へ移動] | | | | |
| **2.5.5** | [削除, 2.2.3 と重複] | | | | |
| **2.5.6** | [文法] パスワードを忘れた場合や他のリカバリパスは、時間ベースの OTP (time-based OTP, TOTP) またはその他のソフトトークン、モバイルパス、または別のオフラインリカバリ機構などのセキュアなリカバリ機構を使用する。 | ✓ | ✓ | ✓ | 640 |
