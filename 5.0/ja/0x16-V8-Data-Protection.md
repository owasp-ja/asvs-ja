...
| **8.2.2** | [修正, 3.2.3 からマージ] ブラウザの記憶域（localStorage、sessionStorage、IndexedDB、Cookie など）に保存されるデータに、クッキーまたは sessionStorage に保存する必要があるセッショントークンを除いて、センシティブなデータが含まれていない。 | ✓ | ✓ | ✓ | 922 |
