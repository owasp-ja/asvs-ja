...
このセクションでは、アプリケーションが内部および外部のサービスと通信する方法についてと、これらのサービスにアクセスできないことによる可用性の損失を防ぐために採用する必要がある技法についてのドキュメント要件を提供します。
...
| **13.2.3** | アプリケーションが外部のデータベースやサービスとの統合のためにパスワードや接続パラメータに関する構成の変更を許可する場合、それらは少なくとも一つの要素での再認証や複数ユーザ承認などの特別なコントロールによって保護されている。 | 2 | v5.0.be-14.7.3 |
...
| **13.3.4** | 重要なシークレットには有効期限が定義されており、組織の脅威モデルとビジネス要件に基づいたスケジュールで入れ替えている。 | 3 | v5.0.be-14.8.3 |
