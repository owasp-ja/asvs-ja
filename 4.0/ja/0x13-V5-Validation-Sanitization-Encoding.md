...
| **5.5.3** | [修正] 信頼できないウライアントとの通信ではデシリアライゼーションが使用されていない。それができない場合は、デシリアライゼーション攻撃を防ぐために、オブジェクトタイプのホワイトリストのみを許可するか、クライアントがデシリアライズ先のオブジェクトタイプを定義できないようにするなど、デシリアライゼーションが安全に実行されるようにしている。 | ✓ | ✓ | ✓ | 502 |
